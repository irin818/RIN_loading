...
cleanup() {
  ...
  if [ -n "$SERVER_PID" ] && kill -0 "$SERVER_PID" 2>/dev/null; then
    echo ""
    echo "Stopping RIN TypeScript fallback Console..."
...
echo "RIN TypeScript fallback Console"
...
echo ""
echo "Running lightweight RIN TypeScript fallback readiness check..."
npm run rin:readiness

echo ""
echo "Starting RIN TypeScript fallback Console at ${CONSOLE_URL}"
echo "Leave this terminal open while using fallback."
...
for _ in {1..40}; do
  if node -e "fetch('${CONSOLE_URL}/api/health').then((r)=>process.exit(r.ok ? 0 : 1)).catch(()=>process.exit(1));" >/dev/null 2>&1; then
    echo "RIN TypeScript fallback Console is ready."
    open "$CONSOLE_URL"
    wait "$SERVER_PID"
    exit $?
  fi
...
echo "RIN TypeScript fallback Console did not become ready at ${CONSOLE_URL}."
