...
- A basic local conversation template using the mock model adapter.
- 一个使用 mock 模型适配器的基础本地对话模板。
...
- Placeholder for future provider-neutral model configuration.
- 未来服务商中立模型配置的占位文件。
...
state files and their schemas before implementing chat or model calls.

如果这些文字有任何不对，下一步应先调整这些本地状态文件及其 schema，再实现
聊天或模型调用。
...
- AI state summary.
- AI 状态摘要。
- Disabled feature gates for external model calls, memory writes, and tool
  execution.
- 外部模型调用、记忆写入和工具执行这些已禁用功能开关。
- Enabled local feature gates for the mock conversation runtime, proposal-only
  memory path, L0 tool execution, and body interaction shell.
- 已启用的本地功能开关包括 mock 对话 runtime、仅提案的记忆路径、L0 工具执行和
  身体交互壳。
