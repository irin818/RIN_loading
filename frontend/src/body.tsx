...
export function BodyPanel(props: {
  ...
  const fallbackActive = forceFallback || model?.activeRenderer !== "cubism-webgl";
  const statusTone = bodyState.warningLevel === "error"
    ? "danger"
    : bodyState.warningLevel === "warning" || model?.status === "partial"
      ? "warn"
      : "ok";
  ...
  return (
    <div className={`body-module activity-${bodyState.activity} ${props.compact ? "compact" : ""}`}>
      ...
      <div className="body-metrics">
        ...
        <MetricCard label="model" value={model?.status ?? "loading"} tone={statusTone} />
      </div>
      <SectionPanel title="Asset Contract" defaultOpen={props.displayMode !== "basic"}>
        {model ? (
          <div className="body-contract">
            ...
            {model.missingRequiredFiles.length ? (
              <div className="tag-row">
                {model.missingRequiredFiles.map((item) => (
...
function BodyStage(props: {
  ...
  if (model.cubismRuntimeActive && !props.forceFallback) {
...
function FallbackAvatarRenderer(props: {
  ...
  const tail = props.assets.tailLarge;
  if (!bust) {
    return (
  ...
  return (
    ...
    >
      {tail ? <img className="body-tail" src={tail} alt="" aria-hidden="true" /> : null}
      <img className="body-bust" src={bust} alt="RIN fallback avatar" />
      <div className="body-face-overlay" aria-hidden="true">
        <span className="body-eye left" />
        <span className="body-eye right" />
        <span className="body-mouth" />
        <span className="body-ai-mark" />
      </div>
      <span className="body-glow" aria-hidden="true" />
    </div>
  );
}

function BodyStatusOverlay(props: {
  payload: BodyStatePayload | null;
  state: BodyRuntimeState;
  ...
  return (
    <aside className={`body-status-overlay ${props.tone}`}>
      <header>
